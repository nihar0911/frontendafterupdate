...
namespace VendorManagement.Web.Services.Invoices
{
    public class GetInvoicesService
    {
        ...
        public async Task<List<InvoiceDto>> GetAsync() { _api.SetAuthHeader(); 
            try
            {
                var r = await _api.Http.GetFromJsonAsync<GetInvoicesResponse>("api/invoices");
                return r?.Invoices ?? new(); 
            } catch
            { 
                return new(); 
            }
        ...
        public async Task<InvoiceDto?> GetByIdAsync(int id) 
        {
            ...
            try
            { 
                var r = await _api.Http.GetFromJsonAsync<GetInvoiceByIdResponse>($"api/invoices/{id}");
                return r?.Invoice; 
            } catch 
            {
                return null;
        ...
        public async Task<byte[]?> DownloadPdfAsync(int invoiceId)
        {
            _api.SetAuthHeader();
            try
            { 
                var resp = await _api.Http.GetAsync($"api/invoices/{invoiceId}/pdf");
